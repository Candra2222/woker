...
var NETWORK_PARSERS = {
  ...
  trafee: /* @__PURE__ */ __name((url) => {
    const p = url.searchParams;
    return {
      network: "trafee",
      click_id: p.get("click_id"),
      // <- diisi dari macro Trafee {track}
      offer_name: p.get("offer_name"),
      // <- dari {name}
      payout: parseFloat(p.get("payout") || "0"),
      // <- dari {sum}
      currency: p.get("currency") || "USD",
      // <- dari {currency}
      country: (p.get("country") || "").toUpperCase() || null,
      // <- dari {country}
  ...
  imonetizeit: /* @__PURE__ */ __name((url) => {
    const p = url.searchParams;
    return {
      network: "imonetizeit",
      click_id: p.get("click_id"),
      ...
      device: p.get("os"),
      // <- dari <os>
      browser: p.get("traffic_type")
      // <- dari <traffic_type> (dipakai ulang kolom browser)
    };
  }, "imonetizeit")
};
var index_default = {
